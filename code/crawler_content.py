...
class crawler_content(crawler_page):
    ...
    def getData(self,text):
          ...
          p_name = re.compile('<span class="gray-a ">(.*?)</span>',re.S)
          name = re.findall(p_name,user_text)[0]
          p_span= re.compile("<span>(.*?)</span>",re.S)
          span = re.findall(p_span,user_text)
          age  = span[1]
          gender = span[0]
          #print gender,age
          p_time = re.compile('<span class="User_newbg User_time">(.*?)</span>',re.S)
          time = re.findall(p_time,user_text)[0]
          ...
          if len(answerDoc)!=0:
             ...
             subsoup = BeautifulSoup(answerDoc,"html.parser")
